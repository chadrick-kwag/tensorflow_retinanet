...
class Match(object):
  ...
  def __init__(self, match_results):
    ...
    the match_results is an array shape (M,) where M is the size of anchors. the values are the gt_box rows that the anchor is assigned to.
    ...
    """
    print("match_results shape:{}".format(match_results.shape))
    print("match_results ndim:{}".format(match_results.ndim))
    print("match_results dtype:{}".format(match_results.dtype))

    if match_results.ndim != 1:
      raise ValueError('match_results should have rank 1')
    if match_results.dtype not in (np.int64, np.int32):
  ...
  def num_matched_columns(self):
    ...
    return self.matched_column_indices().size
  ...
  def num_unmatched_columns(self):
    ...
    return self.unmatched_column_indices().size
  ...
  def num_ignored_columns(self):
    """Returns number (int32 scalar tensor) of matched columns."""
    return self.ignored_column_indices().size
  ...
  def gather_based_on_match(self, input_tensor, unmatched_value,
    ...
    input_tensor: groundtruth_boxes.get() # [N,4] shaped array containing gt_box info
